...
struct RoutineBuilderView: View {
    ...
    struct DraftDay: Identifiable {
        let id = UUID()
        var title: String
        /// nil = sin restriccion (cuerpo completo): cualquier ejercicio vale.
        var allowedMuscleGroups: Set<MuscleGroup>?
        var items: [DraftItem] = []
    }
        ...
        WorkoutGeneratorService.dayTemplates(split: split, dayCount: count)
            .map { template in
                DraftDay(
                    ...
                    allowedMuscleGroups: split == .fullBody ? nil : Set(template.muscleGroups)
                )
            }
    }
    ...
    var body: some View {
        ...
        .sheet(item: Binding(
            ...
            ExercisePickerSheet(
                ...
                allowedMuscleGroups: draftDays[target.dayIndex].allowedMuscleGroups
            ) { exercise in
    ...
    private func save() {
        let routine = WorkoutRoutine(
            name: WorkoutGeneratorService.manualRoutineName(split: splitType),
            splitType: splitType,
