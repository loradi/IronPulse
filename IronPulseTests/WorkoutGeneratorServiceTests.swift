...
struct WorkoutGeneratorServiceTests {
    ...
    @Test func elNombreDeLaRutinaNoMencionaIA() {
        ...
        // Compara contra la funcion real de generacion del nombre (no un
        // literal en espanol) para que el test no dependa del idioma activo
        // del dispositivo/simulador que lo corre - el nombre completo ahora
        // esta localizado, no solo el sufijo del objetivo.
        #expect(routine.name == WorkoutGeneratorService.autoGeneratedRoutineName(goal: .strength))
    ...
    @Test func weekdaysForCountStartingFromRotaLaTablaPreservandoElEspaciado() {
        #expect(WorkoutGeneratorService.weekdaysForCount(3, startingFrom: .monday) == [.monday, .wednesday, .friday])
        #expect(WorkoutGeneratorService.weekdaysForCount(3, startingFrom: .thursday) == [.thursday, .saturday, .monday])
        #expect(WorkoutGeneratorService.weekdaysForCount(2, startingFrom: .sunday) == [.sunday, .wednesday])
        #expect(WorkoutGeneratorService.weekdaysForCount(7, startingFrom: .friday) == [.friday, .saturday, .sunday, .monday, .tuesday, .wednesday, .thursday])
    }
    ...
    @Test func generateRoutineAsignaLosWeekdaysEnOrdenDesdeElLunes() {
        let routine = WorkoutGeneratorService.generateRoutine(for: makeProfile(days: 3), catalog: makeCatalog(), today: .monday)
        let sortedDays = routine.days.sorted { $0.dayNumber < $1.dayNumber }
        #expect(sortedDays.map(\.weekday) == [.monday, .wednesday, .friday])
    }

    @Test func generateRoutineUsaHoyComoPrimerDia() {
        // El dia 1 de una rutina smart debe caer en "hoy", sin importar
        // que dia de la semana sea, en vez de siempre arrancar en lunes.
        let routine = WorkoutGeneratorService.generateRoutine(for: makeProfile(days: 3), catalog: makeCatalog(), today: .thursday)
        let sortedDays = routine.days.sorted { $0.dayNumber < $1.dayNumber }
        #expect(sortedDays.first?.weekday == .thursday)
        #expect(sortedDays.map(\.weekday) == [.thursday, .saturday, .monday])
    }
}
